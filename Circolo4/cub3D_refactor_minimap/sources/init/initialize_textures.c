...
/*   Updated: 2024/06/22 16:31:13 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	*load_xpm_image(t_data *game_data, char *file_path)
{
	...
	texture_buffer = ft_calloc(1, sizeof(*texture_buffer) * game_data->texinfo.size * game_data->texinfo.size);
	if (!texture_buffer)
	...
	while (height < game_data->texinfo.size)
	{
		...
		{
			texture_buffer[height * game_data->texinfo.size + width] = temp_img.addr[height * game_data->texinfo.size + width];
			++width;
		}
		height++;
	}
...
void	initialize_textures(t_data *game_data)
{
	...
	if (!game_data->textures)
		clean_exit(game_data, display_error_message(NULL, ERR_MALLOC, 1));
	game_data->textures[NORTH] = load_xpm_image(game_data, game_data->texinfo.north);
	game_data->textures[SOUTH] = load_xpm_image(game_data, game_data->texinfo.south);
	game_data->textures[EAST] = load_xpm_image(game_data, game_data->texinfo.east);
	game_data->textures[WEST] = load_xpm_image(game_data, game_data->texinfo.west);
}
