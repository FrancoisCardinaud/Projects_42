...
static void	initialize_ray_info(int column, t_ray *raycast,
		t_player *player_data)
{
	initialize_ray(raycast);
	raycast->camera_x = 2 * column / (double)WINDOW_W - 1;
	raycast->dir_x = player_data->dir_x + player_data->plane_vector_x
		* raycast->camera_x;
	raycast->dir_y = player_data->dir_y + player_data->plane_vector_y
		* raycast->camera_x;
