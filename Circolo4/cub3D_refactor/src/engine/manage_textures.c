...
/*   Updated: 2024/07/14 18:47:55 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void initialize_texture_pixels(t_info *game_info) {
    ...
    game_info->texture_pixels = ft_calloc(game_info->win_height + 1, sizeof(*game_info->texture_pixels));
    if (game_info->texture_pixels == NULL) {
        clean_exit(game_info, display_error_message(NULL, MALLOC, 1));
    }

    i = 0;
    while (i < game_info->win_height) {
        game_info->texture_pixels[i] = ft_calloc(game_info->win_width + 1, sizeof(*game_info->texture_pixels));
...
void refresh_texture_pixels(t_info *game_info, t_texinfo *texture_info, t_ray *raycast, int column) {
    ...
    texture_info->pos = (raycast->draw_start - game_info->win_height / 2 + raycast->line_height / 2) * texture_info->step;
