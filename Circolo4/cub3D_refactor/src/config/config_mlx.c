...
/*   Updated: 2024/07/14 18:21:30 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	initialize_image(t_info *game_info, t_img *img, int width, int height)
{
	...
	if (img->img == NULL)
		clean_exit(game_info, display_error_message("mlx", MLX_IMG, 1));
...
void	initialize_graphics(t_info *game_info)
{
	...
	if (!game_info->mlx)
		clean_exit(game_info, display_error_message("mlx", MLX_START, 1));
	game_info->win = mlx_new_window(game_info->mlx, WIN_WIDTH, WIN_HEIGHT,
			"Cub3D");
	if (!game_info->win)
		clean_exit(game_info, display_error_message("mlx", MLX_WIN, 1));
}
