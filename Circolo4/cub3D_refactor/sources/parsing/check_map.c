...
static int	check_map_elements(t_data *data, char **map_tab)
{
	...
	while (map_tab[i] != NULL)
	{
		...
		while (map_tab[i][j])
		{
			...
			if (ft_strchr("NSEW", map_tab[i][j]) && data->character.dir != '0')
				return (display_error_message(data->mapinfo.path, ERR_NUM_PLAYER, FAILURE));
			if (ft_strchr("NSEW", map_tab[i][j]) && data->character.dir == '0')
				data->character.dir = map_tab[i][j];
			j++;
		}
		i++;
	}
...
static int	check_position_is_valid(t_data *data, char **map_tab)
{
	...
	i = (int)data->character.pos_y;
	j = (int)data->character.pos_x;
...
static int	check_character_position(t_data *data, char **map_tab)
{
	int	i;
	int	j;

	if (data->character.dir == '0')
	...
	while (map_tab[i])
	{
		...
		while (map_tab[i][j])
		{
			...
			{
				data->character.pos_x = (double)j + 0.5;
				data->character.pos_y = (double)i + 0.5;
				map_tab[i][j] = '0';
			}
			j++;
		}
...
int	validate_map(t_data *data, char **map_tab)
{
	...
	if (check_map_elements(data, map_tab) == FAILURE)
		return (FAILURE);
	if (check_character_position(data, map_tab) == FAILURE)
		return (FAILURE);
