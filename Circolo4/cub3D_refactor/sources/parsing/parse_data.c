...
/*   Updated: 2024/04/17 03:29:43 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_number_of_lines(char *path)
{
	...
	if (fd < 0)
		display_error_message(path, strerror(errno), errno);
	else
	{
...
static void	fill_tab(int row, int column, int i, t_data *data)
{
	...
	while (line != NULL)
	{
		...
		if (!data->mapinfo.file[row])
		{
			...
			return (free_memory((void **)data->mapinfo.file));
		}
		while (line[i] != '\0')
...
void	parse_data(char *path, t_data *data)
{
	...
	if (data->mapinfo.fd < 0)
		display_error_message(path, strerror(errno), FAILURE);
	else
	{
