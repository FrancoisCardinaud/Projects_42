...
/*   Updated: 2024/06/22 17:31:39 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	calculate_map_height(t_data *game_data, char **file_lines, int start_index)
{
	int	row;
	int	col;
	...
	{
		col = 0;
		while (file_lines[row][col] == ' ' || file_lines[row][col] == '\t' || file_lines[row][col] == '\r'
		|| file_lines[row][col] == '\v' || file_lines[row][col] == '\f')
			col++;
...
static int	populate_map_array(t_mapinfo *map_info, char **map_array, int current_index)
{
	int		row;
	int		col;
	...
	while (row < map_info->height)
	{
		...
		while (map_info->file[current_index][col] && map_info->file[current_index][col] != '\n')
		{
...
static int	extract_map_info(t_data *game_data, char **file_lines, int start_index)
{
	game_data->mapinfo.height = calculate_map_height(game_data, file_lines, start_index);
	game_data->map = malloc(sizeof(char *) * (game_data->mapinfo.height + 1));
	if (!game_data->map)
		return (display_error_message(NULL, ERR_MALLOC, FAILURE));
	if (populate_map_array(&game_data->mapinfo, game_data->map, start_index) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}
	...
	while (game_data->map[row])
	{
		...
		while (game_data->map[row][++col])
		{
			if (game_data->map[row][col] == ' '
				&& col != game_data->map[row][ft_strlen(game_data->map[row]) - 1])
