...
/*   Updated: 2024/03/09 22:38:56 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	fill_direction_textures(t_texture_data *textures, char *line, int j)
{
...
static int	ignore_whitespaces_get_info(t_data *data, char **map, int i, int j)
{
	...
	if (ft_isprint(map[i][j]) && !ft_isdigit(map[i][j]))
	{
		if (map[i][j + 1] && ft_isprint(map[i][j + 1])
			&& !ft_isdigit(map[i][j]))
		{
			if (fill_direction_textures(&data->texture_data, map[i], j) == ERR)
				return (display_error_message(data->mapinfo.path, ERR_TEX_INVALID, FAILURE));
			return (BREAK);
		}	
		else
		{
			if (fill_textures_color_data(data, &data->texture_data, map[i], j) == ERR)
				return (FAILURE);
			return (BREAK);
		}	
	}
	...
	{
		if (generate_map(data, map, i) == FAILURE)
			return (display_error_message(data->mapinfo.path, ERR_INVALID_MAP, FAILURE));
		return (SUCCESS);
	}
	return (CONTINUE);
}
