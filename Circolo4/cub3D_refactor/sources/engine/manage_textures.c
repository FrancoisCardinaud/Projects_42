...
/*   Updated: 2024/06/22 17:56:57 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	refresh_texture_pixels(t_data *game_data, t_texinfo *texture_info, t_ray *raycast, int column)
{
	int			row;
	int			color;
	...
	while (row < raycast->draw_end)
	{
		...
		color = game_data->textures[texture_info->index][texture_info->size * texture_info->y + texture_info->x];
