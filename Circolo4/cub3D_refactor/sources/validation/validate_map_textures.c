...
/*   Updated: 2024/06/22 17:32:32 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	validate_rgb_values(int *rgb_values)
{
	...
	while (i < 3)
	{
		if (rgb_values[i] < 0 || rgb_values[i] > 255)
			return (display_error_with_value(rgb_values[i], ERR_TEX_RGB_VAL, FAILURE));
		i++;
	}
	return (SUCCESS);
}
...
int	validate_textures(t_data *game_data, t_texinfo *texture_info)
{
	if (!texture_info->north || !texture_info->south || !texture_info->west
		|| !texture_info->east)
		return (display_error_message(game_data->mapinfo.path, ERR_TEX_MISSING, FAILURE));
	if (!texture_info->floor || !texture_info->ceiling)
		return (display_error_message(game_data->mapinfo.path, ERR_COLOR_MISSING, FAILURE));
