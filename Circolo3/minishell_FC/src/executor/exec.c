...
void	child_execute(t_shell_data *shell_data, t_command *cmd_data, int len, t_list *cmd_list)
{
	signal(SIGINT, SIG_DFL);
	signal(SIGQUIT, SIG_DFL);
	if (!is_command_builtin(cmd_data) && cmd_data->full_cmd)
		execve(cmd_data->full_path, cmd_data->full_cmd, shell_data->env_data);
	...
	{
		print_env_matrix(shell_data->env_data, 1, 1);
		global_stat = 0;
	}
}
...
void	*child_routine(t_shell_data *shell_data, t_list *cmd_list, int pipe_fd[2])
{
	t_command	*cmd_data;
	int		len;
	...
	clear_command_list(&shell_data->cmds, free_content_data);
	exit(global_stat);
}

void	execute_child(t_shell_data *shell_data, t_list *cmd_list, int pipe_fd[2])
{
	pid_t	child_pid;
...
void	*evaluate_fork(t_shell_data *shell_data, t_list *cmd_list, int pipe_fd[2])
{
	t_command	*cmd_data;
	DIR		*directory;
