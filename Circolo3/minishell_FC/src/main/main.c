...
static t_prompt	setup_vars(t_prompt shell_data, char *temp_str, char **args)
{
	...
	shell_data.envp = mini_setenv("PWD", temp_str, shell_data.envp, 3);
	free(temp_str);
	temp_str = mini_getenv("SHLVL", shell_data.envp, 5);
	if (!temp_str || ft_atoi(temp_str) <= 0)
		level_str = ft_strdup("1");
	else
		level_str = ft_itoa(ft_atoi(temp_str) + 1);
	free(temp_str);
	shell_data.envp = mini_setenv("SHLVL", level_str, shell_data.envp, 5);
	free(level_str);
	temp_str = mini_getenv("PATH", shell_data.envp, 4);
	if (!temp_str)
		shell_data.envp = mini_setenv("PATH", \
		"/usr/local/sbin:/usr/local/bin:/usr/bin:/bin", shell_data.envp, 4);
	free(temp_str);
	temp_str = mini_getenv("_", shell_data.envp, 1);
	if (!temp_str)
		shell_data.envp = mini_setenv("_", args[0], shell_data.envp, 1);
	free(temp_str);
	return (shell_data);
}
...
int	main(int arg_count, char **args, char **envp)
{
	...
	while (args && arg_count)
	{
		...
		temp_str = mini_getprompt(shell_data);
		if (temp_str)
