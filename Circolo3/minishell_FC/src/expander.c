...
/*   Updated: 2023/10/23 17:50:06 by fcardina         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*expand_directory(char *input, int index, int quote_flags[2],
		char *variable)
{
	char	*directory;
	char	*temp;
	...
	while (input && input[++index])
	{
		...
		quote_flags[1] = (quote_flags[1] + (!quote_flags[0]
					&& input[index] == '\"')) % 2;
		if (!quote_flags[0] && !quote_flags[1] && input[index] == '~'
			&& (index == 0 || input[index - 1] != '$'))
		{
			...
			free(temp);
			free(directory);
			return (expand_directory(input, index + ft_strlen(variable) - 1,
					quote_flags, variable));
		}
	}
	free(variable);
	return (input);
		...
		if (!quote_flags[0] && input[index] == '$' && input[index + 1]
			&& ((ft_strchars_i(&input[index + 1], "/~%^{}:; ")
					&& !quote_flags[1]) || (ft_strchars_i(&input[index + 1],
						"/~%^{}:;\"") && quote_flags[1])))
			return (expand_variables(retrieve_sub_var(input, ++index,
						shell_data), -1, quote_flags, shell_data));
	}
	return (input);
}
